...
pub fn get_GuepardOS_pi_path() -> Option<String> {
    which::which("GuepardOS-pi").ok().map(|p| p.to_string_lossy().into_owned())
}

pub fn is_GuepardOS_pi_installed() -> bool {
    get_GuepardOS_pi_path().is_some()
}
